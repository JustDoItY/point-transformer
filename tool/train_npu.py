...
def main():
    ...
    if args.manual_seed is not None:
        ...
        torch.npu.manual_seed(args.manual_seed)
        torch.npu.manual_seed_all(args.manual_seed)
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    criterion = nn.CrossEntropyLoss(ignore_index=args.ignore_label).npu()
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            checkpoint = torch.load(args.resume, map_location=lambda storage, loc: storage.npu())
...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (coord, feat, target, offset) in enumerate(train_loader):  # (n, 3), (n, c), (n), (b)
        ...
        coord, feat, target, offset = coord.npu(non_blocking=True), feat.npu(non_blocking=True), target.npu(non_blocking=True), offset.npu(non_blocking=True)
